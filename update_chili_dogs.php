...
echo "<h1>🌭 Updating Chili Hot Dogs Image</h1>\n";
...
try {
    ...
    $stmt = $pdo->prepare("SELECT id, name, image FROM recipes WHERE name LIKE '%Chili Hot Dog%'");
    $stmt->execute();
    $recipe = $stmt->fetch();
    ...
    if ($recipe) {
        ...
        // Update with a proper chili hot dogs image - actual hot dogs with chili
        $newImageUrl = 'https://images.unsplash.com/photo-1542574271-7f3b92e6c821?ixlib=rb-4.0.3&auto=format&fit=crop&w=800&q=80';
        ...
        echo "<p style='color: #e9ecef; font-size: 14px;'>New Image URL: {$newImageUrl}</p>\n";
        echo "</div>\n";
        
        echo "<p style='color: #51cf66;'>✅ Successfully updated Chili Hot Dogs image!</p>\n";
        
    } else {
        echo "<p style='color: #ff6b6b;'>❌ Chili Hot Dogs recipe not found!</p>\n";
    }
    
    echo "<p><a href='country.php?country=America' style='color: #74c0fc;'>🇺🇸 View American Recipes</a></p>\n";
