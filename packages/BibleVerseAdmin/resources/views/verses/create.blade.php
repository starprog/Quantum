...
<div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
    <div class="p-6">
        <div class="flex justify-between items-center mb-6">
            <h2 class="text-2xl font-semibold text-gray-800">Add New Verse</h2>
            <a href="{{ route('admin.verses.index') }}" class="text-gray-600 hover:text-gray-900">
                ← Back to Verses
            </a>
        </div>

        <form action="{{ route('admin.verses.store') }}" method="POST" class="space-y-6">
            @csrf
            ...
            <div>
                ...
                <textarea name="verse" id="verse" rows="6" 
                    ...
                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 @error('verse') border-red-500 @enderror">{{ old('verse') }}</textarea>
                @error('verse')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
